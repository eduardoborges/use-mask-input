...
import useHookFormMaskShadcn from './useHookFormMaskShadcn';
...
describe('useHookFormMaskShadcn', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('returns a function', () => {
    ...
    const { result } = renderHook(
      () => useHookFormMaskShadcn(registerFn as UseFormRegister<FieldValues>),
    );
  ...
  it('throws when registerFn is missing', () => {
    const { result } = renderHook(
      () => useHookFormMaskShadcn(undefined as unknown as UseFormRegister<FieldValues>),
    );
    expect(() => result.current('field' as never, '999-999'))
      .toThrowError('registerFn is required');
  });

  it('registers field with mask and applies it to the input element', () => {
  ...
  it('handles null ref from register', () => {
    ...
    const { result } = renderHook(
      () => useHookFormMaskShadcn(registerFn as unknown as UseFormRegister<FieldValues>),
    );
  ...
  it('handles null input element', () => {
    ...
    expect(applyMaskToElement).not.toHaveBeenCalled();
    expect(prevRef).toHaveBeenCalledWith(null);
  });
  ...
  it('returns stable ref callback identity across calls', () => {
  ...
  it('invalidates the ref cache when registerFn changes', () => {
    const registerFn1 = makeRegisterFn('phone');
    const registerFn2 = makeRegisterFn('phone');

    const { result, rerender } = renderHook(
      ({ fn }) => useHookFormMaskShadcn(fn as UseFormRegister<FieldValues>),
